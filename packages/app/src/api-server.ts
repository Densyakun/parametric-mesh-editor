...
// This runs as a separate server on port 3001
...
// In production, use Supabase to validate keys
const apiKeys = new Map<string, string>(); // key -> userId
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  // API Key validation (skip for docs endpoint)
  const authHeader = req.headers.authorization;
  const apiKey = authHeader?.replace('Bearer ', '');

  if (path !== '/api/docs' && path !== '/api/health') {
    if (!apiKey) {
      sendJSON(res, 401, { error: 'API key required. Pass it as Authorization: Bearer <key>' });
      return;
    }

    // For demo, accept any key starting with 'mna_'
  ...
  try {
    ...
    if (path === '/api/health' && method === 'GET') {
      ...
      if (!body?.dsl) {
        sendJSON(res, 400, { error: 'Missing "dsl" field in request body' });
        return;
      }
...
server.listen(PORT, () => {
  console.log(`MeshNative API server running on http://localhost:${PORT}`);
